    ...
    for seed in range(24601, 24611):
        ...
        if veh_num == 2:
            ...
            thedata = list(zip(depot.tolist(),  # Depot location
                               cust.tolist(),
                               d.tolist(),
                               np.full((dataset_size, 2), cap).tolist()
                                ))
            data.append(thedata)
...
def eval_dataset(dataset_path, width, softmax_temp, opts):
    ...
    if opts.multiprocessing:
        ...
        dataset =  HCVRP.make_dataset( filename=dataset_path, num_samples=opts.val_size, offset=opts.offset)
        # dataset = generate_hcvrp_data(128, 40, 3)
    ...
    if opts.o is None:
        results_dir = os.path.join(opts.results_dir, HCVRP.NAME, dataset_basename)
...
if __name__ == "__main__":
    ...
    opts.obj = 'min-sum'
    dataset_path = r"D:\projects\HCVRP_DRL\fleet_v2\data\hcvrp\hcvrp_v2_20_seed24610.pkl"
    opts.model = r"D:\projects\HCVRP_DRL\fleet_v2\outputs\hcvrp_20\run_20221012T164442\epoch-49.pt"
    ...
    for width in widths:
        # for dataset_path in opts.datasets:
           x, y, z = eval_dataset(dataset_path, width, opts.softmax_temperature, opts)
    a = 5