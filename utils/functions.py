...
def load_problem(name):
    from fleet_v2.problems import hcvrp
    problem = {
        'hcvrp': hcvrp
    }.get(name, None)
...
def load_model(path, obj, epoch=None):
    from fleet_v2.nets.attention_model import AttentionModel
    from fleet_v2.nets.pointer_network import PointerNetwork
...
def sample_many(inner_func, get_cost_func, input, batch_rep=1, iter_rep=1):
    ...
    for i in range(iter_rep):
        _log_p, _log_p_veh, pi, veh_list, tour_1, tour_2= inner_func(input, )
        #print('pi', pi.size())

        cost = get_cost_func(input, pi, veh_list, tour_1, tour_2)
        mask =None
