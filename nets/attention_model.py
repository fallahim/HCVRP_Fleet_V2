...
class AttentionModel(nn.Module):
    ...
    def forward(self, input, return_pi=False):
        ...
        _log_p, log_p_veh, pi, veh_list, tour_1, tour_2 = self._inner(input, embeddings)  # _log_p: [batch_size, graph_size+1, graph_size+1], pi:[batch_size, graph_size+1]

        cost = HCVRP.get_costs(input, self.obj, pi, veh_list, tour_1, tour_2)  # mask is None, cost:[batch_size]
        mask = None
    ...
    def select_veh(self, input, state, sequences, embeddings, obj, veh_list, tour_1, tour_2):
        ...
        if sequences:
            ...
            mean_tour = torch.zeros([batch_size, 2 * embed_dim]).float().cuda()
    ...
    def _inner(self, input, embeddings):
        ...
        while not (self.shrink_size is None and state.all_finished()):
            veh, log_p_veh = self.select_veh(input, state, sequences, embeddings, self.obj, veh_list, tour_1, tour_2)  # [batch_size, 1]
        ...
        return torch.stack(outputs, 1), torch.stack(outputs_veh, 1), torch.stack(sequences, -1).squeeze(-2), veh_list, \
               torch.stack(tour_1, -1), torch.stack(tour_2, -1)
    ...
    def sample_many(self, input, batch_rep=1, iter_rep=1):
        ...
        return sample_many(
            ...
            lambda input, pi, veh_list, tour_1, tour_2: HCVRP.get_costs(input[0], self.obj, pi, veh_list, tour_1, tour_2),  # Don't need embeddings as input to get_costs
            ...
            if from_depot:
                ...
                return torch.cat(  # [batch_size, num_veh, embed_dim+1]
                    (
                        ...
                        torch.tensor(HCVRP.VEHICLE_CAPACITY)[None, :, None].cuda() - torch.zeros_like(
                ...
                return torch.cat(  # [batch_size, num_veh, embed_dim+1]
                    (
                        ...
                        (torch.tensor(HCVRP.VEHICLE_CAPACITY)[None, veh].cuda() - state.used_capacity[torch.arange(batch_size), veh]).transpose(0, 1).unsqueeze(-1)
                    ),
                    -1
                )
