...
class HCVRP(object):
    ...
    VEHICLE_CAPACITY = [25., 50.]

    @staticmethod
    def get_costs(dataset, obj, pi, veh_list, tour_1, tour_2):
        # if obj == 'min-max':
        #     SPEED = [1, 1]
        # if obj == 'min-sum':
        SPEED = [1/4, 1/5]
        ...
        total_dis = torch.cat((total_dis_1, total_dis_2), -1)

        if   obj[0] == 'min-sum':
            return torch.sum(total_dis, dim=1)
        elif obj[0] == 'min-max':
                return torch.max(total_dis, dim=1)[0]
